...
const BOT_NAME_FANCY = 'Naruto Md';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://files.catbox.moe/f10i6l.jpg',
...
async function handleMessageRevocation(socket, number) {
  socket.ev.on('messages.delete', async ({ keys }) => {
    ...
    const message = formatMessage('*🗑️ 𝘔𝘢𝘴𝘴𝘦𝘨𝘦 𝘋𝘭𝘦𝘢𝘵𝘦𝘥*', `*𝘈 𝘮𝘦𝘴𝘴𝘦𝘨𝘦 𝘋𝘭𝘦𝘵𝘦𝘥 𝘍𝘳𝘰𝘮 𝘠𝘰𝘶𝘳 𝘊𝘩𝘢𝘵*\n*📄 𝘍𝘳𝘰𝘮:* ${messageKey.remoteJid}\n*⏳ 𝘋𝘭𝘦𝘵𝘦 𝘛𝘪𝘮𝘦:* ${deletionTime}`, BOT_NAME_FANCY);
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    let botName = cfg.botName || 'NARUTO MD MINI';
...
case 'gpt': {
  try {
    ...
    const prompt = `
*You should act like a real girl. If there are only emojis, send only emojis... 
Your name is ${botName} , and if someone asks you "Who made you?" you only say "Brother Hirun Vikasitha made me* User Message: ${q}
    `;
 ...
 case 'weather': {
  try {
    // 🗣️ Messages in English
    const messages = {
      noCity: "❗ *Please provide a city name!* \n📄 *Usage: .weather [city name]*",
      weather: (data) => `
*☘️ Naruto md Weather Report ☘️*
...
`,
    ...
    const caption = messages.weather(data);
    const bannerUrl = "https://files.catbox.moe/f10i6l.jpg"; // 🖼️ Banner image

    await socket.sendMessage(sender, {
      image: { url: bannerUrl },
      caption: caption,
      footer: "🌦️ *Weather Information*",
      buttons: [
        { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📄 𝘔𝘢𝘪𝘯 𝘔𝘦𝘯𝘶" }, type: 1 },
...
END:VCARD`
    ...
    try {
        ...
        if (!result || !result.code) {
            return await socket.sendMessage(sender, {
                text: '*❌ Failed to retrieve pairing code. Please check the number.6'
            }, { quoted: msg });
        }
...
*1. ⏳ Pair this code within 1 minute.6
...
case 'fbd': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'NARUTO MINI BOT AI';
...
END:VCARD`
        ...
        const caption = `📥 *Naruto Md Apk Download* 📥

📱 *${result.name}*\n\n` +
                        `*🆔 Package:* \`${result.package}\`\n` +
                        `*📦 Size:* ${result.size}\n` +
                        `*🕒 Last Update:6 ${result.lastUpdate}\n\n` +
                        ` ${botName}`;
...
case 'alive': {
  try {
    ...
    const text = `
*HI 👋 NARUTO MD MINI BOT USER 😼💗 I AM ALIVE NOW 🥷💗*
...
`;
    ...
    // 🌀 Video URL (Round PTV)
    const videoUrl = 'https://files.catbox.moe/gblpiv.mp4';

    // Send round video first
    await socket.sendMessage(sender, {
      video: { url: videoUrl },
      mimetype: 'video/mp4',
      ptv: true
    ...
    // Wait 1.5 seconds before sending text message
...
case 'bots': {
  try {
    ...
    let text = `🤖 *𝘈𝘤𝘵𝘪𝘷𝘦 𝘚𝘦𝘴𝘴𝘪𝘰𝘯𝘴= ${botName}*\n\n`;
    text += `📊 *𝘛𝘰𝘵𝘢𝘭 𝘈𝘤𝘵𝘪𝘷𝘦 𝘚𝘦𝘴𝘴𝘪𝘰𝘯𝘴 =* ${activeCount}\n\n`;
...
case 'song': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '*𝙉𝘼𝙍𝙐𝙏𝙊 - 𝙈𝘿 -  𝙈𝙄𝙉𝙄*';
...
END:VCARD`
    ...
    try {
        ...
        const caption = `
 *🎵 `𝘕𝘢𝘳𝘶𝘵𝘰 𝘔𝘥 𝘔𝘪𝘯𝘪 𝘉𝘰𝘵 𝘚𝘰𝘯𝘨 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥𝘦𝘳` 🎵*

*╭──────────●●➤*
*┣ 🍀Title ⦁* ${title}
*┣ ⏱️Duration ⦁* ${duration || 'N/A'}
*┣ 🔊Quality ⦁* ${quality}
*┣ 🖇️Url ⦁* ${videoUrl}
*╰──────────●●➤*

*𝐑ᴇᴘʟʏ 𝐓ʜᴇ 𝐁ᴇʟᴏᴡ 𝐍ᴜᴍʙᴇʀ ↓*

*1️⃣⦁ 📄 MP3 As Document*
*2️⃣⦁ 🎧 MP3 As Audio*
*3️⃣⦁ 🎤 MP3 As Video Note*
...
`;  
    ...
    // --- Send round PTV video ---  
    const videoUrl = 'https://files.catbox.moe/eyhonn.mp4';  
    await socket.sendMessage(sender, {  
      video: { url: videoUrl },  
      mimetype: 'video/mp4',  
      ptv: true  
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '*𝙉𝘼𝙍𝙐𝙏𝙊 - 𝙈𝘿 -  𝙈𝙄𝙉𝙄*';
...
END:VCARD`
    ...
    const text = `
*HI 👋 NARUTO MD MINI BOT USER 😼💗*

*╭─「 𝐁ᴏᴛ 𝐃ᴇᴛᴀɪʟꜱ 」 ──●●➤* 
*┣ 🗣️𝘉𝘰𝘵 𝘕𝘢𝘮𝘦 =* ${title}
*┣ 🥷𝘖𝘸𝘯𝘦𝘳 =* ${config.OWNER_NAME || 'HIRUN'}
*┣ 🪀𝘖𝘸𝘯𝘦𝘳 𝘕𝘣 =* 02529242
*┣ 📡𝘝𝘦𝘳𝘴𝘪𝘰𝘯 =* ${config.BOT_VERSION || '0.0001+'}
*┣ 🎲𝘗𝘭𝘢𝘵𝘧𝘰𝘳𝘮 =* ${process.env.PLATFORM || 'HEROKU'}
*┣ ⏳𝘜𝘱𝘵𝘪𝘮𝘦 =* ${hours}h ${minutes}m ${seconds}s
*╰──────────●●➤*
...
`.trim();
    ...
    // 🌀 PTV video (Round video before menu)
    const videoUrl = 'https://files.catbox.moe/zjlinb.mp4';
    ...
    // Wait few seconds before showing the menu
    await new Promise(r => setTimeout(r, 1500));

    const defaultImg = 'https://files.catbox.moe/f10i6l.jpg';
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || '𝗡𝗔𝗥𝗨𝗧𝗢 𝗠𝗗';
...
`.trim();
    ...
    const bannerUrl = "https://files.catbox.moe/f10i6l.jpg";
...
case 'creative': {
  ...
  try {
    ...
    const title = userCfg.botName || 'NARUTO MD MINI';
...
`.trim();
    ...
    const buttons = [
      { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📄 𝘔𝘢𝘪𝘯 𝘔𝘦𝘯𝘶" }, type: 1 },
      { buttonId: `${config.PREFIX}download`, buttonText: { displayText: "📥 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘔𝘦𝘯𝘶" }, type: 1 }
    ];
    ...
    try { 
      await socket.sendMessage(sender, { text: '❌ *Failed to show creative menu.*' }, { quoted: msg }); 
    } catch(e){}
  }
  break;
}
...
`.trim();
    ...
    try { 
      await socket.sendMessage(sender, { text: '*❌ Failed to show tools menu.*' }, { quoted: msg }); 
    } catch(e){}
  }
  break;
}
...
`.trim();
    ...
    try { 
      await socket.sendMessage(sender, { text: '❌ Failed to show settings menu.' }, { quoted: msg }); 
    } catch(e){}
  }
  break;
}
...
`.trim();
    ...
    // 🖼️ Add banner image here
    const imageUrl = 'https://files.catbox.moe/f10i6l.jpg';
...
END:VCARD` } }
        ...
        const buttons = [{ buttonId: `${config.PREFIX}img ${q}`, buttonText: { displayText: "🖼️ 𝘕𝘦𝘹𝘵 𝘐𝘮𝘢𝘨𝘦" }, type: 1 }];
...
//💐💐💐💐💐💐
  ...
  if (!jid) {
    ...
    const title = userCfg.botName || 'NARUTO MINI BOT AI';
...
case 'grouplist': {
  try {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || BOT_NAME_FANCY || "NARUTO MD MINI";
...
END:VCARD`
        ...
        const caption = `📥 *`𝘕𝘢𝘳𝘶𝘵𝘰 𝘔𝘥 𝘔𝘪𝘯𝘪 𝘔𝘦𝘢𝘥𝘪𝘢𝘍𝘪𝘳𝘦 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥`*📥
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of Naruto Md Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:naruto2008@gmail.com\n' + // Email
      'ADR;type=WORK:;;Ratnapura;;Sri Lanka\n' + // Address
      'URL:https://github.com\n' + // Website
      'TEL;type=CELL;type=VOICE;waid=94702529242\n' + // WhatsApp Number
      'TEL;type=CELL;type=VOICE;waid=94705629091\n' + // Second Number (Owner)
      'END:VCARD';
...
END:VCARD`
    ...
    await socket.sendMessage(sender, { 
      text: '*❌ Internal Error. Please try again later.*',
      buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📄 𝘉𝘰𝘵 𝘐𝘯𝘧𝘰' }, type: 1 }]
    });
  }
  break;
}
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'NARUTO MD MINI'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname Hiru X Md`' }, { quoted: shonux });
  }
...
async function deleteSessionAndCleanup(number, socketInstance) {
  ...
  try {
    ...
    try {
      const ownerJid = `${config.OWNER_NUMBER.replace(/[^0-9]/g,'')}@s.whatsapp.net`;
      const caption = formatMessage('🗣️ OWNER NOTICE — SESSION REMOVED', `Number: ${sanitized}\nSession removed due to logout.\n\nActive sessions now: ${activeSockets.size}`, BOT_NAME_FANCY);
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: 'NARUTO  FREE BOT', activesession: activeSockets.size });
});
...
router.get('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    if (sock) await sock.sendMessage(jidNormalizedUser(sock.user.id), { image: { url: config.RCD_IMAGE_PATH }, caption: formatMessage('☘️ CONFIG UPDATED', 'Your configuration has been successfully updated!', BOT_NAME_FANCY) });
